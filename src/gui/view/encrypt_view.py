"""
View para encriptação de ficheiros (AES-GCM e 3DES-CBC).
"""
...
from crypto.symmetric import encrypt_file_aes, encrypt_file_3des
from gui.components.file_picker import FilePicker


class EncryptView(ctk.CTkFrame):
    """View para encriptar ficheiros."""
    ...
    def __init__(self, master, **kwargs):
        ...
        desc = ctk.CTkLabel(
            self, 
            text="Encripte ficheiros usando AES-256-GCM ou 3DES-CBC",
        ...
        self.password_entry.grid(row=5, column=0, padx=10, pady=(0, 10), sticky="ew")
        
        # Confirmar password
        pwd_confirm_label = ctk.CTkLabel(main_frame, text="Confirmar Password:", anchor="w")
        pwd_confirm_label.grid(row=6, column=0, padx=10, pady=(10, 5), sticky="w")
        
        self.password_confirm_entry = ctk.CTkEntry(main_frame, show="*", placeholder_text="Confirme a password")
        self.password_confirm_entry.grid(row=7, column=0, padx=10, pady=(0, 15), sticky="ew")
        ...
        self.encrypt_button.grid(row=0, column=0, padx=5, sticky="ew")
        ...
        self.status_label.grid(row=9, column=0, padx=10, pady=(0, 10), sticky="w")
    ...
    def _encrypt(self):
        ...
        if not input_path:
            messagebox.showerror("Erro", "Por favor, selecione o ficheiro de entrada.")
            return
        
        if not os.path.exists(input_path):
            messagebox.showerror("Erro", "O ficheiro de entrada não existe.")
            return
        ...
        if not password:
            messagebox.showerror("Erro", "Por favor, digite uma password.")
            return
        ...
        try:
            self.status_label.configure(text="A encriptar...", text_color="orange")
            self.update()
            
            # Encriptar com o algoritmo selecionado
            if algorithm == "AES-256-GCM":
                encrypt_file_aes(input_path, output_path, password)
            else:  # 3DES-CBC
                encrypt_file_3des(input_path, output_path, password)
            
            self.status_label.configure(text="✅ Ficheiro encriptado com sucesso!", text_color="green")
            messagebox.showinfo("Sucesso", f"Ficheiro encriptado com {algorithm}!\nGuardado em: {output_path}")
            
        except Exception as e:
            self.status_label.configure(text="❌ Erro na encriptação", text_color="red")
            messagebox.showerror("Erro", f"Erro ao encriptar o ficheiro:\n{str(e)}")
